...
import {dataValidator, multiPatternValidator, multiPatternValidatorSelect} from "../../form/Validator/validator";
...
export class BambinoComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, protected formD: ControlFormDirective, private InscrizioneBambinoService : BambinoService, private accessService:  AccessService, private router: Router) {
  ...
  removeDynamicControl(type: string) {
...
createBambino( ) {
  if(this.is_url_ceck()=="Nuovo"){
    ...
    this.formGroup.removeControl("Patologie");
    this.formGroup.removeControl("Terapisti");
    this.formGroup.addControl("Patologie", this.fb.control(this.bambino?.patologie ? cloneDeep(this.bambino.patologie) : ["Patologie"], this.formD.form[5].input.validator));
    this.formGroup.addControl('Terapisti', this.fb.control(this.bambino?.terapista_associati? cloneDeep(this.bambino.terapista_associati) : ['Terapisti'], this.formD.form[6].input.validator));
  }else{
    this.is_modifica=false;
  }
  }
        ...
        this.formD.form.push(
          ...
          },
          {
            ...
            input: {
              ...
              disabled: is_visualizza,
              value: this.bambino?.terapista_associati ? [...this.bambino.patologie] : ['Patologie'],
              validator: [
  ...
  set_initialize_formgroup(){
   this.formGroup.addControl("Descrizione", this.fb.control(this.bambino?.descrizione? this.bambino.descrizione : ""))
   this.formGroup.addControl('AssociaTerapisti', this.fb.control(this.bambino?.terapista_associati.length !== 0 ? this.bambino?.terapista_associati : false));
   this.formGroup.addControl('ValidazioneTerapista', this.fb.control(this.bambino?.controllo_terapista ? this.bambino.controllo_terapista : true));
